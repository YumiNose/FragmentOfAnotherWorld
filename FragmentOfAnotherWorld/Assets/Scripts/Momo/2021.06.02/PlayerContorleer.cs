...
public class PlayerContorleer : MonoBehaviour
{
    [SerializeField]
    ...
    public GameObject beam;//ビーム

    public bool isDead = false;  // 死んでいるかどうか
    ...
    void Start()
    {
        ...
        if( Physics2D.gravity.y > 0)//重力が上向きなら
        {
            Physics2D.gravity *= -1;//重力を下向きにする
        }
    }
    void Update()
    {
        if((isDead == true) || (char2.GetComponent<PlayerContorleer>().isDead == true))
        {
            if(isDead == false)
            {
                Movesub();
        ...
        if (Input.GetKeyDown(KeyCode.F) || Input.GetButtonDown("Change"))
        {
    ...
    void Move()
    {
        ...
        // ジャンプ
        if ((Input.GetKeyDown("space") || Input.GetButtonDown("Jump") )&& this.isGround)
        {
            // 上に向けて力を加える
        ...
        //ビーム   
        if ((Input.GetKeyDown(KeyCode.X)) || Input.GetButtonDown("Atack"))
        {
            var _clone = Instantiate(beam);
    ...
    void Fall()
    {
        ...
        if (Mathf.Abs(gameObject.transform.position.y - Camera.main.transform.position.y) >  Mathf.Abs(bottomY))
        {
            Invoke("Restart", 2);
